				...
				if (path.data[0] == '*')
				{
					if (m_Texture[path.data] == NULL)
					{
						...
						D3DX11CreateShaderResourceViewFromMemory(
							Renderer_DirectX11::Instance()->GetDevice(), (const unsigned char*)m_AiScene->mTextures[id]->pcData,
...
// スケルタルメッシュ削除関数
//=============================================================================
void  Component_SkeletalMesh::UnloadFBX()
{
	for (unsigned int m = 0; m < m_AiScene->mNumMeshes; m++)
	{
...
void Component_SkeletalMesh::SetNextAnimationName(std::string _name, unsigned int _frame)
{
	...
	//フレーム最大数を超えないようにする
	m_Animation[_name].frame = _frame < m_Animation[_name].animation->mAnimations[0]->mNumChannels ?_frame : 0;
}
